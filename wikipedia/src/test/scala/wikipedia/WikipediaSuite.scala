...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'occurrencesOfLang' should work for RDD containing several articles") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val articles = Seq(
      WikipediaArticle("Java", "Java is a language"),
      WikipediaArticle("Scala", "Scala is a language that uses the JVM used by Java"),
      WikipediaArticle("Groovy", "Is Groovy a programming language?"),
      WikipediaArticle("C++", "C++ inspired Java")
    )
    val rdd = sc.parallelize(articles)
    assert(occurrencesOfLang("Java", rdd) == 3)
  ...
  test("'rankLangs' should work for RDD with several elements") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java", "JavaScript")
    val articles = List(
      WikipediaArticle("1", "Scala is great"),
      WikipediaArticle("2", "Another language is JavaScript"),
      WikipediaArticle("3", "Java is OK, but Scala is cooler"),
      WikipediaArticle("4", "Scala was created later than Java")
    )
    val rdd = sc.parallelize(articles)
    val ranked = rankLangs(langs, rdd)
    assert(ranked.head._1 == "Scala")
    assert(ranked(1)._1 == "Java")
    assert(ranked(2)._1 == "JavaScript")
  }
